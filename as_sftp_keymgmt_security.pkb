CREATE OR REPLACE PACKAGE BODY as_sftp_keymgmt_security IS
    ...
    BEGIN
        IF v_depth < 4 THEN
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.GET_PRIV_KEY.');
        ...
        */
        ...
        IF v_owner = '&&compile_schema.' AND v_name = 'AS_SFTP_KEYMGMT.GET_PRIV_KEY' THEN 
            RETURN NULL;
        ELSE 
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.GET_PRIV_KEY.');
    ...
    BEGIN
        IF v_depth < 4 THEN
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.INSERT_PRIV_KEY.');
            dbms_output.put_line('call stack less than 4: '||v_depth);
            DBMS_OUTPUT.put_line('Not allowing rows to be selected.');
            RETURN '1=0';
        END IF;
        ...
        */
        ...
        --dbms_output.put_line('owner: '||v_owner||' name: '||v_name||' lineno: '||v_lineno||' caller_t: '||v_caller_t);
        --dbms_output.put_line(DBMS_UTILITY.format_call_stack);
        IF v_owner = '&&compile_schema.' AND v_name = 'AS_SFTP_KEYMGMT.INSERT_PRIV_KEY' THEN 
            RETURN NULL;
        ELSE 
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.INSERT_PRIV_KEY.');
            DBMS_OUTPUT.put_line('was called by owner: '||v_owner||' name: '||v_name);
    ...
    BEGIN
        IF v_depth < 4 THEN
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.UPDATE_PRIV_KEY.');
            dbms_output.put_line('call stack less than 4: '||v_depth);
            DBMS_OUTPUT.put_line('Not allowing rows to be selected.');
            RETURN '1=0';
        END IF;
        ...
        */
        ...
        --dbms_output.put_line('owner: '||v_owner||' name: '||v_name||' lineno: '||v_lineno||' caller_t: '||v_caller_t);
        --dbms_output.put_line(DBMS_UTILITY.format_call_stack);
        IF v_owner = '&&compile_schema.' AND v_name = 'AS_SFTP_KEYMGMT.UPDATE_PRIV_KEY' THEN 
            RETURN NULL;
        ELSE 
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.UPDATE_PRIV_KEY.');
            DBMS_OUTPUT.put_line('was called by owner: '||v_owner||' name: '||v_name);
    ...
    BEGIN
        IF v_depth < 4 THEN
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.DELETE_PRIV_KEY.');
            dbms_output.put_line('call stack less than 4: '||v_depth);
            DBMS_OUTPUT.put_line('Not allowing rows to be selected.');
            RETURN '1=0';
        END IF;
        ...
        */
        ...
        --dbms_output.put_line('owner: '||v_owner||' name: '||v_name||' lineno: '||v_lineno||' caller_t: '||v_caller_t);
        --dbms_output.put_line(DBMS_UTILITY.format_call_stack);
        IF v_owner = '&&compile_schema.' AND v_name = 'AS_SFTP_KEYMGMT.DELETE_PRIV_KEY' THEN 
            RETURN NULL;
        ELSE 
            DBMS_OUTPUT.put_line('security check found not called by &&compile_schema..AS_SFTP_KEYMGMT.DELETE_PRIV_KEY.');
            DBMS_OUTPUT.put_line('was called by owner: '||v_owner||' name: '||v_name);
